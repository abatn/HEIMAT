...
interface Stop { id: string; name: string; latitude: number; longitude: number; stop_type: string; }
...
export class MobilityService {
  ...
  async getNearbyStops(lat: number, lng: number, radiusMeters: number = 1000): Promise<Stop[]> {
    return query<Stop>(
      `SELECT id, name, latitude, longitude, stop_type FROM stops
       WHERE (6371000 * acos(cos(radians($2)) * cos(radians(latitude)) * cos(radians(longitude) - radians($1)) + sin(radians($2)) * sin(radians(latitude)))) < $3
       ORDER BY (6371000 * acos(cos(radians($2)) * cos(radians(latitude)) * cos(radians(longitude) - radians($1)) + sin(radians($2)) * sin(radians(latitude))))
       LIMIT 10`, [lng, lat, radiusMeters]);
  }
  ...
  async searchStops(query_text: string): Promise<Stop[]> {
    return query<Stop>('SELECT * FROM stops WHERE name ILIKE $1 ORDER BY name LIMIT 20', [`%${query_text}%`]);
  }

  async geocodeAddress(address: string): Promise<{ lat: number; lng: number; display_name: string }[]> {
    try {
      const response = await axios.get(`${this.nominatimUrl}/search`, { params: { q: address, format: 'json', limit: 5, countrycodes: 'de' }, headers: { 'User-Agent': 'HEIMAT-App/1.0' } });
  ...
  async getRoute(from: { lat: number; lng: number }, to: { lat: number; lng: number }): Promise<any> {
    try {
      const response = await axios.get(`${this.osrmUrl}/route/v1/driving/${from.lng},${from.lat};${to.lng},${to.lat}`, { params: { overview: 'full', geometries: 'geojson' } });
