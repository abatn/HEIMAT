...
class MobilityProvider extends ChangeNotifier {
  ...
  Future<void> loadNearbyStops(double lat, double lng,
    ...
    try {
      final response = await _api.get(
        '/api/mobility/stops?lat=$lat&lng=$lng&radius=$radius',
      );
      final stops =
  ...
  Future<void> searchStops(String query) async {
    ...
    try {
      final response = await _api.get(
        '/api/mobility/stops/search?q=${Uri.encodeComponent(query)}',
      );
      _searchResults =
          (response['stops'] as List).map((s) => Stop.fromJson(s)).toList();
    } catch (e) {
  ...
  Future<void> geocodeAddress(String address) async {
    ...
    try {
      final response = await _api.get(
        '/api/mobility/geocode?address=${Uri.encodeComponent(address)}',
      );
